...
class RuleParserSpec extends UnitSpec {
    "A rule" should "be defined in Rule.scala" in {
        // Derived rules
        assert(rule.parse("LEM").get === Rule.Derived(Special.LEM))
        assert(rule.parse("MT(1 ,1   )").get === Rule.Derived(Special.MT(1, 1)))
        assert(rule.parse("PC  (4,5)").get === Rule.Derived(Special.PC(4, 5)))
        assert(rule.parse("refl").get === Rule.Derived(Special.Refl))
        assert(
          rule.parse("=sub (    1, 4)").get === Rule.Derived(
            Special.EqSub(1, 4)
          )
        )
        assert(rule.parse("sym    (1  )").get === Rule.Derived(Special.Sym(1)))
        assert(rule.parse("given").get === Rule.Derived(Special.Given))
        assert(rule.parse("ass").get === Rule.Derived(Special.Ass))
        assert(rule.parse("tick ( 9)").get === Rule.Derived(Special.Tick(9)))

...
class ParserSpec extends UnitSpec {
    ...
    "A function application" should "be a function followed by (, some lterms and a )" in {
        ...
        assert(funcAp.parse("baz (bar (foo(x,y,z)    , kk), h, j  ,  k,l, bar (foo(x,y  ,z)))").get === nested3)
    }
    ...
    "An LTerm" should "be a funcap or a variable" in {
        ...
        assert(lterm.parse("baz (bar (foo(x,y,z)    , kk), h, j  ,  k,l, bar (foo(x,y  ,z)))").get === nested3)
    }
}

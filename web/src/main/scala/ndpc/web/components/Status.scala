...
object Status:
     ...
     * Returns the CM6 extension and a <span> DOM element to append to the toolbar.
     * The extension is a ViewPlugin that updates the span on every doc change.
     */
    ...
    private def setStatus(span: dom.html.Span, ok: Boolean, count: Int): Unit =
        if ok then
            span.textContent = "✓ Proof correct"
            span.className   = "status-indicator status-ok"
        else
            span.textContent = s"✗ $count error${if count == 1 then "" else "s"}"
